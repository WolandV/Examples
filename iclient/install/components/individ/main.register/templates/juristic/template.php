...
foreach ($arResult["SHOW_FIELDS"] as $val)	{
	...
	if (is_array($val)) {	
		...
		if ($val["FIELD_NAME"]=="UF_BOSS") $arResult_new["SHOW_FIELDS"][160]=$val;	
		if ($val["FIELD_NAME"]=="UF_RAB") $arResult_new["SHOW_FIELDS"][170]=$val;	
        if ($val["FIELD_NAME"]=="UF_CATEGORY") $arResult_new["SHOW_FIELDS"][145]=$val;    
	}
	else 
	{		
		...
		if ($val=="NAME") $arResult_new["SHOW_FIELDS"][40]=$val;
		if ($val=="LAST_NAME") $arResult_new["SHOW_FIELDS"][50]=$val;
				
		if ($val=="PERSONAL_PHONE") $arResult_new["SHOW_FIELDS"][60]=$val;
        if ($val=="PERSONAL_FAX") $arResult_new["SHOW_FIELDS"][70]=$val;        
		if ($val=="PERSONAL_PHOTO") $arResult_new["SHOW_FIELDS"][75]=$val;		
		
		if ($val=="PERSONAL_STREET") $arResult_new["SHOW_FIELDS"][100]=$val;
		if ($val=="PERSONAL_MAILBOX") $arResult_new["SHOW_FIELDS"][110]=$val;
		if ($val=="PERSONAL_ZIP") $arResult_new["SHOW_FIELDS"][120]=$val;
		
		
		if ($val=="EMAIL") $arResult_new["SHOW_FIELDS"][130]=$val;	
		if ($val=="PERSONAL_WWW") $arResult_new["SHOW_FIELDS"][140]=$val;				
		
		if ($val=="PERSONAL_NOTES") $arResult_new["SHOW_FIELDS"][150]=$val;				
...
<?
$showCategorySectionTitle=false;
foreach ($arResult["SHOW_FIELDS"] as $FIELD):?>

	<?if($FIELD=="LOGIN"):?><div class="title">Авторизационные данные</div><?endif;?>
	<?if($FIELD=="NAME"):?><div class="title top_margin">Публичные профессиональные данные</div><?endif;?>
    ...
    <div id="div-<?=$FIELD["FIELD_NAME"]?>"<?if ($notshow):?> style="display: none;"<?endif?>>
		...
		<div class="fname">			
                ...
                */
				...
				if (is_array($FIELD)):?>
					<?if($FIELD["FIELD_NAME"]=="UF_CATEGORY"  && !$showCategorySectionTitle):?>
						<div style="margin:1em 0"><strong>Данные, относящиеся к конференции:</strong></div>
                        <?$showCategorySectionTitle=true?>
					<?endif;?>					
					<?=$FIELD["EDIT_FORM_LABEL"]?>:					
				<?else:?>		
					<?if ($FIELD=="PERSONAL_NOTES" && !$showCategorySectionTitle):?>
						<div style="margin:1em 0"><strong>Данные, относящиеся к конференции:</strong></div>
                        <?$showCategorySectionTitle=true?>
					<?endif;?>
...
{
	?>
		<div class="title top_margin"><?=GetMessage("REGISTER_CAPTCHA_TITLE")?></div>
		<div>
...
<input type="hidden" name="UF_UR" value="1"/>

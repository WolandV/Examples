...
foreach ($arResult["SHOW_FIELDS"] as $val){
	if (is_array($val)) {
		...
		if ($val["FIELD_NAME"]=="UF_INTEREST") $arResult_new["SHOW_FIELDS"][65]=$val;
		
		if ($val["FIELD_NAME"]=="UF_SECTION") $arResult_new["SHOW_FIELDS"][220]=$val;
		if ($val["FIELD_NAME"]=="UF_FORMA") $arResult_new["SHOW_FIELDS"][230]=$val;
		if ($val["FIELD_NAME"]=="UF_TECH") $arResult_new["SHOW_FIELDS"][240]=$val;
		if ($val["FIELD_NAME"]=="UF_HOTEL") $arResult_new["SHOW_FIELDS"][250]=$val;
		if ($val["FIELD_NAME"]=="UF_DATECOMMING") $arResult_new["SHOW_FIELDS"][260]=$val;
		if ($val["FIELD_NAME"]=="UF_DATEDEP") $arResult_new["SHOW_FIELDS"][270]=$val;
		if ($val["FIELD_NAME"]=="UF_DATACOMMING") $arResult_new["SHOW_FIELDS"][280]=$val;	
        
        if ($val["FIELD_NAME"]=="UF_SADDRESS") $arResult_new["SHOW_FIELDS"][300]=$val;    
        if ($val["FIELD_NAME"]=="UF_SPHONE") $arResult_new["SHOW_FIELDS"][301]=$val;    
        if ($val["FIELD_NAME"]=="UF_SFAX") $arResult_new["SHOW_FIELDS"][302]=$val;    
        if ($val["FIELD_NAME"]=="UF_SWWW") $arResult_new["SHOW_FIELDS"][303]=$val;    
	}
	else
	{
		...
		if ($val=="SEARCH_COMPANY") $arResult_new["SHOW_FIELDS"][70]=$val;
		if ($val=="WORK_POSITION") $arResult_new["SHOW_FIELDS"][71]=$val;
        if ($val=="WORK_PROFILE") $arResult_new["SHOW_FIELDS"][66]=$val;
		if ($val=="PERSONAL_PHOTO") $arResult_new["SHOW_FIELDS"][67]=$val;
		
		if ($val=="PERSONAL_STREET") $arResult_new["SHOW_FIELDS"][100]=$val;
		if ($val=="PERSONAL_PHONE") $arResult_new["SHOW_FIELDS"][110]=$val;
		if ($val=="EMAIL") $arResult_new["SHOW_FIELDS"][120]=$val;	
		if ($val=="PERSONAL_WWW") $arResult_new["SHOW_FIELDS"][130]=$val;				
		
		if ($val=="WORK_NOTES") $arResult_new["SHOW_FIELDS"][290]=$val;
	}
}
...
<?foreach ($arResult["SHOW_FIELDS"] as $FIELD):?>
	...
	<?if($FIELD=="SEARCH_COMPANY"):?><div class="title top_margin">Публичные профессиональные данные</div><?endif;?>
   ...
   */
    ...
    <div id="div-<?=$FIELD["FIELD_NAME"]?>"<?if ($notshow):?> style="display: none;"<?endif?>>
		...
		<div class="<?=$f_class?>"<?if ($FIELD["FIELD_NAME"]=="UF_INTEREST"):?> style="overflow-x: auto;"<?endif?>><?
	switch ($FIELD)
	{
		...
		case "SEARCH_COMPANY":
            ...
            if(strlen(trim($_REQUEST["SEARCH_COMPANY"])) <= 0)
                $_REQUEST["SEARCH_COMPANY"] = $placeholder;
			?>                           
			<div style="position: relative; margin-bottom: 8px;"> 
			<div class="f_req2">
				<input class="inputtext" size="30" type="text"  name="SEARCH_COMPANY" value="<?=htmlspecialchars($_REQUEST["SEARCH_COMPANY"])?>"
					type="text" id="search_company"	

					onfocus="if (this.value=='<?=$placeholder?>') this.value='';" 
					onblur="if(this.value =='') this.value='<?=$placeholder?>'" 
					onkeyup="if(this.value!='<?=$placeholder?>') sr_ajax_show_help(this.value, event)"											
					autocomplete="off"/>
			</div> <script>placeholder="<?=$placeholder?>";</script>
            ...
            <div id="PH">Если вы не нашли свою компанию, то можете <a href="#" onclick="return SetCompanyFromSearch(0);">создать новую</a>.</div>
			<div id="OurCompany" <?if ($_REQUEST["WAS_FORM_COMPANY"] != 'Y'):?> style="display: none;"<?endif;?>>
             <?if ($_REQUEST["WAS_FORM_COMPANY"] == 'Y'):?> 
                <?require_once($_SERVER['DOCUMENT_ROOT'].'/bitrix/templates/individ/ajax/ajax_company_set.php');?>
             <?endif?>
            </div>
            <?/*
...
<br/>
<p>
Домашний адрес, телефон и e-mail открытой публикации не подлежат, а служат для связи с Оргкомитетом. Заполняя настоящую анкету, Вы подтверждаете свое согласие на получение на свои адреса информационных рассылок Оргкомитета и информационных рассылок Конгресса конференций «ИТО».
</p>
<script type="text/javascript">
